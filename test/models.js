...
describe('Shopify Model', () => {
	it('should create a new shop', done => {
		const ShopifyMock = sinon.mock(
			new Shopify({ shopDomain: 'minhpt@bravebits.vn', accessToken: 'minh-token' })
		);
		const user = ShopifyMock.object;

		ShopifyMock.expects('save').yields(null);
	...
	it('should return error if shop is not created', done => {
		const ShopifyMock = sinon.mock(
			new User({ shopDomain: 'minhpt@bravebits.vn', accessToken: 'minh-token' })
		);
		const shop = ShopifyMock.object;
		const expectedError = {
			name: 'ValidationError'
		};

		ShopifyMock.expects('save').yields(expectedError);
